...
"""
@description: Axis Camera Parameter Class
...
"""
Parameter class for groups, such as Motion.M0, or StreamProfile.S1
Holds individual parameters
"""
class AxisParameterGroup:
	def __init__(self):
		return
			

"""        
x = AxisParameter('Event')
p = AxisParameterList()
p.setIP('10.0.0.1')
p.setCredentials('root','pass')
p.determineVapixVersion()
v=p.getParameters([x])
i=0
print len(v)
for x in v:
	...
	if x.name == "Event.E1.Image":
		x.value = 1
		y = p.updateParameters([x])
